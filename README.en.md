...
- Audio lossifier: damage uploaded audio with clipping, sample-rate crush, bit-depth reduction, narrow-band filtering, and time smear.
...
├── app.js                   # Noise synthesis, mixer DSP, audio noising/lossifier, export encoding, i18n
├── index.html               # App UI
...
Audio lossifier also processes uploaded files locally, but it does not layer palette noise. It simulates meme-style low-bitrate, clipped, phone-repost, and bitcrush damage chains. The output uses the selected sample rate, format, and normalization setting.
